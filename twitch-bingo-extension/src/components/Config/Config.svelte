<script lang="ts">
    ...
    function parseEntriesText(content: string)
    {
        var newEntries:BingoEditableEntry[] = new Array(0);
    ...
    function onEntriesUpload(evt: Event): void
    {
        ...
        {
            let entriesText = evt.clipboardData?.getData("text") ?? null
            if (entriesText)
            {
